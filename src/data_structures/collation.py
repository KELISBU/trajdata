...
def _collate_data(elems, dtype = None):
    if hasattr(elems[0], "__collate__"):
        ...
        return torch.as_tensor(np.stack(elems), dtype=dtype)
...
def agent_collate_fn(
    ...
    for out_key in batch_elems[0].extras.keys():
        cur_extras_element = batch_elems[0].extras[out_key]
        is_nested = isinstance(cur_extras_element, dict)
        if is_nested:
            for key in batch_elems[0].extras[out_key].keys():
                if key in ['map_polylines', 'map_polylines_mask', 'map_polylines_center']:
                    val_list = [torch.from_numpy(batch_elem.extras[out_key][key]) for batch_elem in batch_elems]
                    extras[key] = merge_batch_by_padding_2nd_dim(
                        val_list)
                else:
                    extras[key] = _collate_data(
                    [batch_elem.extras[out_key][key] for batch_elem in batch_elems],
                    dtype=torch.float if key != "has_lane" else torch.bool
                    )
        elif out_key in ['closest_lane_points', 'full_fut_traj', 'full_fut_valid']:
            closest_lane_points_list = [batch_elem.extras[out_key] for batch_elem in batch_elems]
            extras[out_key] = pad_sequence(
                closest_lane_points_list, batch_first=True, padding_value=np.nan
            )
        else:
            extras[out_key] = _collate_data(
                [batch_elem.extras[out_key] for batch_elem in batch_elems],
                dtype=torch.float if out_key != "has_lane" else torch.bool
            )
...
def scene_collate_fn(
    ...
    for out_key in batch_elems[0].extras.keys():
        ...
        elif out_key in ['closest_lane_point', 'full_fut_traj', 'full_fut_valid']:
            closest_lane_points_list = [batch_elem.extras[out_key] for batch_elem in batch_elems]
            extras[out_key] = pad_sequence(
                closest_lane_points_list, batch_first=True, padding_value=np.nan
            )
