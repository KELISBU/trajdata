...
class WaymoScenarios:
    def __init__(
        ...
        split_path = Path("/net/acadia3a/data/wjchang/waymo_splitted") / (self.name + "_splitted")
        if split or not split_path.is_dir():
            self.split_scenarios()
        else:
            self.num_scenarios = len(os.listdir(split_path))
            
        # scenario_id_map = read_scenario_id_map(split_path / "/scenario_id_map", self.name)
        
        # if not scenario_id_map:
        #     scenario_id_map = create_scenarios_id_maps(split_path / "/scenario_id_map", self.name)
        # else:
        #     print(f"Using existing scenario ID to index mapping from {split_path}/scenario_id_map/{self.name}_scenario_id_map.json")
    ...
    def split_scenarios(
        ...
        splitted_dir: Path = Path("/net/acadia3a/data/wjchang/waymo_splitted") / f"{self.name}_splitted"
...
def extract_vectorized(
    ...
    for map_feature in tqdm(
        ...
        if map_feature.WhichOneof("feature_data") == "lane":
            ...
            #driveway,speed_bump
            continue
    #add dummy lane if no map elements 
    if not vec_map.has_elements:
        dummy_road_lane = RoadLane(
            id="dummy_road_lane",
            center=Polyline(np.array([[0, 0, 0], [0, 0, 0]])),
            left_edge=None,
            right_edge=None,
        )
        max_pt = np.array([0.1, 0.1, 0.1])
        max_pt = np.array([0, 0, 0])
        vec_map.add_map_element(dummy_road_lane)
