...
def calc_stats(
    ...
    x_prime = torch.diff(positions[:, :, 0], dim=1) / dt
    y_prime = torch.diff(positions[:, :, 1], dim=1) / dt

    # Second derivatives (acceleration components)
    x_double_prime = torch.diff(x_prime, dim=1) / dt
    y_double_prime = torch.diff(y_prime, dim=1) / dt

    # Curvature calculation
    curvature = (x_prime[:, :-1] * y_double_prime - y_prime[:, :-1] * x_double_prime) / \
                torch.pow(x_prime[:, :-1] ** 2 + y_prime[:, :-1] ** 2, 1.5)
    # Replace NaN and Inf values
    curvature[torch.isnan(curvature)] = 0.0
    curvature[torch.isinf(curvature)] = 0.0

    # Adding the curvature histogram
    stats = {
        ...
        "curvature": torch.histogram(curvature, bins.get("curvature", torch.linspace(-10, 10, 21)))
    }
